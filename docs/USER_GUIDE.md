...
- **No `Test Licence` button.** The licence is HMAC-verified locally on every request; there's no Magento-side endpoint to call. The Module Status banner at the top of the admin config section tells you whether the current key is valid for the current host.
